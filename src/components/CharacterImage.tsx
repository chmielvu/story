...
export const CharacterImage = defineComponent({
    ...
    template: `
      <div class="relative w-full aspect-video bg-black flex items-center justify-center rounded-lg overflow-hidden shadow-2xl" :style="{ 'box-shadow': '0 25px 50px -12px var(--shadow-color)' }">
        <div v-if="isLoading || isEditing" class="absolute z-10 inset-0 flex items-center justify-center bg-black/50">
          <div class="w-16 h-16 border-4 border-t-transparent border-accent-primary rounded-full animate-spin"></div>
          <span v-if="isLoading" class="absolute mt-20 text-sm text-text-muted">Altering reality...</span>
          <span v-if="isEditing" class="absolute mt-20 text-sm text-text-muted">Refining vision...</span>
        </div>
        <img v-if="imageUrl" :src="imageUrl" class="w-full h-full object-cover transition-opacity duration-1000" :class="{ 'opacity-50': isLoading || isEditing, 'opacity-100': !isLoading && !isEditing }"/>
